...
let listUrl = "https://comic.naver.com/webtoon/finish"; //사이트

let result = [];
request(listUrl, (err, res, body) => {
    const $ = cheerio.load(body); 
    let titleList = $(".list_area li a"); //타이틀 리스트를 가져온다.
    for(let i = 0; i < titleList.length; i++){
        let title = titleList.eq(i).attr("title");
        ...
        let id = href.substring(startIdx+1); // startIndex 한칸 앞에서 시작하며 endIdx전까지 있는 titleId를 가져옴
        if(title !== undefined && title !== null && title !== "") {
            result[title.replace(/ /gi,"")] = id;
        }
    }
